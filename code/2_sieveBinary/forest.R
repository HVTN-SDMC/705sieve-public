#' @param table A table to be plotted
#' @param header.plot Header of the plot to be shown
#' @param cols_CI columns for mean, lower bound of the CI, upper bound of the CI
#' @param xlim the x-axis limits of the forest plot
#' @param x_ticks_at the x-axis ticks
#' @param xlab x-axis label
#' @param ci_cols colors for the rows of the forest plot
#' @param nrows_underline rows where extra underlines to be added
forestWrap <- function(table, cols_CI, header.plot, xlim, x_ticks_at, xlab, ci_cols, 
                       nrows_underline = NULL, insertHeaderText = NULL,
                       insertHeaderTextCol = NULL, footnote = NULL){
  cols_upperCI <- max (cols_CI)
  cols_lowerCI <- min (cols_CI)
  cols_total <- dim(table)[2]
  dt <- table[,1:cols_upperCI]
  #insert empty spaces for plotting the confidence interval
  dt$` ` <- paste(rep(" ", 20), collapse = " ")#this determines how wide the forestplot is
  if(cols_total>cols_upperCI){
    dt <- cbind(dt, table[,(cols_upperCI+1):cols_total])
  }
  
  header <- c(header.plot[1: (min(cols_CI)-1)], "mean", "lower", "upper","", header.plot[-(1: (min(cols_CI)-1))])
  colnames(dt) = header
  ...
  tm <- forest_theme(base_size = 10,
                     ...
                     header_size = 10.1,
                     footnote_cex = 1)
  p <- forest(dt[,-c(cols_lowerCI : cols_upperCI)], #omit the columns of mean, lower, upper
              est = dt$mean,
              lower = dt$lower, 
              upper = dt$upper,
              sizes = 2,
              ci_cols = ci_cols,
              ci_column = cols_lowerCI,
              ref_line = 0,
              xlim = xlim,
              xlab = xlab,
              ticks_at = x_ticks_at,
  ...
  if(!is.null(nrows_underline)){
    for(i in 1:length(nrows_underline)){
      p <- add_underline(p, row = nrows_underline[i], part = "body", gp = gpar(lwd=2))
    }
  }
  if(!is.null(insertHeaderText)){
    p <- insert_text(p,
                text = insertHeaderText,
                col = insertHeaderTextCol,
                part = "header",
                just = "center",
                gp = gpar(fontface = "bold", fontsize = 10.1))
    p <- add_underline(p,row = 1, col = insertHeaderTextCol, part = "header", gp = gpar(lwd=2))
  }
  return(p)
}
...
forest <- function(data,
  ...
  # Add padding
  gt <- gtable_add_padding(gt, unit(1, "mm"))
...
#' @param base_size The size of header
#'  for details.
#'
